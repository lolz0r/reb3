        ...
        if (mNextNativeState == NativeState.RESUMED) {
            if (mSurface.mIsSurfaceReady && mHasFocus && mIsResumedCalled) {
                if (mSDLThread == null) {
                    ...
                    // B3: the harness main() carries a large stack frame
                    // (fine on the desktop 8 MB main stack); the default
                    // Java-thread stack overflowed at the first callee write
                    // (tombstone: "stack pointer is not in a rw map").
